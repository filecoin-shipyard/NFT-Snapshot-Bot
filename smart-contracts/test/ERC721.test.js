...
const TwitterThreadSnapshot = contract.fromArtifact('TwitterThreadSnapshot');
...
describe('ERC721', function () {
  ...
  const name = 'TwitterThreadSnapshot';
  const symbol = 'THRD';
  ...
  const randomFilecoinHash = 'xyz';

  beforeEach(async function () {
    this.token = await TwitterThreadSnapshot.new();
  });
  ...
  describe('metadata', function () {
    ...
    describe('token URI', function () {
      beforeEach(async function () {
        await this.token.mint(randomFilecoinHash, owner);
      });
  ...
  context('with minted tokens', function () {
    beforeEach(async function () {
      await this.token.mint(randomFilecoinHash, owner);
      await this.token.mint(randomFilecoinHash+'ddd', owner);
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevert(
          this.token.mint(randomFilecoinHash, ZERO_ADDRESS), 'ERC721: mint to the zero address'
      );
    });

    context('with minted token', async function () {
      beforeEach(async function () {
        ({ logs: this.logs } = await this.token.mint(randomFilecoinHash,owner));
      });
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expectRevert(this.token.mint(randomFilecoinHash, owner), 'Mint: Thread snapshot has already been tokenised.');
      });
    });
  });
});